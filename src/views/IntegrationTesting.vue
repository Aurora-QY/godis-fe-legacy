      ...
      <el-card style="display: flex; flex-wrap: wrap">
        ...
        <div style="margin: 20px; display: inline-block">
          ...
          <div style="height: 200px; width: 200px; display: inline-block">
            <PieChartComponent :chart-data="getPassRateData()" />
            <div style="vertical-align: top; margin-left: 20px">
              <h3>测试用例总数: {{ getTotalTestCases() }}</h3>
            </div>
            <div style="vertical-align: top; margin-left: 20px">
              <h3>总通过率: {{ getOverallPassRate() }}%</h3>
            </div>
          </div>
        </div>
      </el-card>
  ...
  methods: {
    ...
    async executeAllTestCases() {
      for (const testCase of this.allTestCases) {
        ...
        testCase.isPass = testCase.commands.every((command) => command.isPass === 'Pass') ? 'Pass' : 'Fail'
      }
      // 强制更新通过率和命令统计数据
      this.updateCharts()
    },
